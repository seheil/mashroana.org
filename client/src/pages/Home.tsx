...
export default function Home() {
  ...
  return (
    <div className="overflow-hidden bg-[#fbfcf9] text-slate-900">
      ...
      <section className="relative -mt-10 px-4"><div className="mx-auto grid max-w-6xl grid-cols-2 gap-3 rounded-3xl bg-white p-4 shadow-xl shadow-emerald-950/10 md:grid-cols-4 md:gap-4 md:p-6">{impactCards.map((card) => <article key={card.label} className="rounded-2xl bg-[#f4f8f3] p-4 md:p-5"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-emerald-700 text-xl font-black text-white">{card.icon}</span><p className="mt-4 text-2xl font-black text-[#123c2c] md:text-3xl">{card.value.toLocaleString("ar-EG")}</p><p className="mt-1 text-xs leading-5 text-slate-600 md:text-sm">{card.label}</p></article>)}</div></section>
