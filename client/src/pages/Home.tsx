  ...
  return (
    <div className="overflow-hidden bg-[#fbfcf9] text-slate-900">
      <section className="relative isolate min-h-[680px] overflow-hidden bg-[#113d2c] px-4 pb-16 pt-24 text-white md:pt-32">
        ...
        <div className="mx-auto grid max-w-6xl gap-12 lg:grid-cols-[1.18fr_0.82fr] lg:items-center"><div><p className="mb-5 inline-flex rounded-full border border-emerald-100/25 bg-emerald-300/10 px-4 py-2 text-sm font-bold text-emerald-100">{content.hero.eyebrow}</p><h1 className="max-w-4xl text-4xl font-black leading-[1.24] tracking-tight md:text-6xl">{content.hero.title}</h1><p className="mt-7 max-w-2xl text-lg leading-8 text-emerald-50/90">{content.hero.description}</p><div className="mt-9 flex flex-wrap gap-3"><a href="#donate" className="rounded-xl bg-[#e8c57a] px-6 py-3.5 font-black text-[#173a2c] transition hover:bg-[#f4d78f]">{content.hero.primaryCtaLabel}</a><Link href="/partnerships" className="rounded-xl border border-white/35 px-6 py-3.5 font-bold text-white transition hover:bg-white/10">{content.hero.secondaryCtaLabel}</Link></div><div className="mt-10 flex flex-wrap gap-x-8 gap-y-3 text-sm text-emerald-100">{(content.hero.trustPoints || []).map((point) => <span key={point}>✓ {point}</span>)}</div></div><aside className="relative rounded-[2rem] border border-white/15 bg-white/10 p-7 shadow-2xl backdrop-blur-sm"><p className="text-sm font-bold text-emerald-200">{content.hero.sideLabel}</p><p className="mt-4 text-2xl font-black leading-10">{content.hero.sideTitle}</p><div className="mt-8 border-t border-white/15 pt-6"><p className="text-sm leading-7 text-emerald-50/85">{content.hero.sideDescription}</p><Link href="/transparency" className="mt-5 inline-block font-bold text-[#f4d78f] hover:underline">{content.hero.sideCtaLabel}</Link></div></aside></div>
      </section>
