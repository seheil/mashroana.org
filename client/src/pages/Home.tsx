...
export default function Home() {
  ...
  const sectionBlocks: Record<HomepageSectionId, ReactNode> = {
    impact: content.impact.enabled ? (
      <section key="impact" className="relative -mt-10 px-4" aria-labelledby="impact-summary-title">
        <h2 id="impact-summary-title" className="sr-only">{content.impact.title || "مؤشرات الأثر المنشورة"}</h2>
        <div className="mx-auto grid max-w-6xl grid-cols-2 gap-3 rounded-3xl bg-white p-4 shadow-xl shadow-emerald-950/10 md:grid-cols-4 md:gap-4 md:p-6">
          {impactCards.map((card) => {
            const verified = hasVerifiedImpactValue(card.value);
            return <article key={card.label} className="rounded-2xl bg-[#f4f8f3] p-4 md:p-5"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-emerald-700 text-xl font-black text-white" aria-hidden="true">{card.icon}</span><p className="mt-4 text-2xl font-black text-[#123c2c] md:text-3xl" aria-live="polite">{verified ? card.value.toLocaleString("ar-EG") : "—"}</p><p className="mt-1 text-xs leading-5 text-slate-600 md:text-sm">{card.label}</p>{!verified && <p className="mt-1 text-xs font-semibold text-slate-500">قيد التوثيق</p>}</article>;
          })}
        </div>
      </section>
    ) : null,
    priorities: content.priorities.enabled ? <div key="priorities"><DonationPriorityNotice /></div> : null,
    ...
    partnerships: content.partnerships.enabled ? (
      <section key="partnerships" className="bg-[#eef5ee] px-4 py-20"><div className="mx-auto max-w-6xl"><div className="rounded-3xl bg-[#153f2e] p-8 text-white shadow-xl md:max-w-3xl"><p className="font-bold text-emerald-200">{content.partnerships.eyebrow}</p><h2 className="mt-3 text-3xl font-black leading-tight">{content.partnerships.title}</h2><p className="mt-4 leading-8 text-emerald-50/90">{content.partnerships.description}</p><Link href="/partnerships" className="mt-7 inline-block rounded-xl bg-[#e8c57a] px-5 py-3 font-bold text-[#153f2e]">{content.partnerships.ctaLabel}</Link></div></div></section>
    ) : null,
    media: content.media.enabled ? (
  ...
  return (
    <div className="overflow-hidden bg-[#fbfcf9] text-slate-900">
      <section className="relative isolate min-h-[680px] overflow-hidden bg-[#113d2c] px-4 pb-16 pt-24 text-white md:pt-32">
        <div className="absolute inset-0 -z-10 bg-[radial-gradient(circle_at_12%_18%,rgba(166,220,179,0.28),transparent_32%),radial-gradient(circle_at_89%_75%,rgba(231,191,112,0.2),transparent_30%)]" /><div className="absolute left-[7%] top-[16%] -z-10 h-40 w-40 rounded-full border border-emerald-100/15" /><div className="absolute bottom-[-140px] right-[-100px] -z-10 h-[410px] w-[410px] rounded-full border-[36px] border-emerald-200/10" />
        <div className="mx-auto grid max-w-6xl gap-12 lg:grid-cols-[1.18fr_0.82fr] lg:items-center"><div><p className="mb-5 inline-flex rounded-full border border-emerald-100/25 bg-emerald-300/10 px-4 py-2 text-sm font-bold text-emerald-100">{content.hero.eyebrow}</p><h1 className="max-w-4xl text-4xl font-black leading-[1.24] tracking-tight md:text-6xl">{content.hero.title}</h1><p className="mt-7 max-w-2xl text-lg leading-8 text-emerald-50/90">{content.hero.description}</p><div className="mt-9 flex flex-wrap gap-3"><a href="#donate" className="rounded-xl bg-[#e8c57a] px-6 py-3.5 font-black text-[#173a2c] transition hover:bg-[#f4d78f]">{content.hero.primaryCtaLabel}</a><Link href="/partnerships" className="rounded-xl border border-white/35 px-6 py-3.5 font-bold text-white transition hover:bg-white/10">{content.hero.secondaryCtaLabel}</Link></div><div className="mt-10 flex flex-wrap gap-x-8 gap-y-3 text-sm text-emerald-100"><span>✓ تواصل مباشر مع المؤسسة</span><span>✓ محتوى مؤسسي قابل للمراجعة</span><span>✓ خصوصية واحترام للمستفيدين</span></div></div><aside className="relative rounded-[2rem] border border-white/15 bg-white/10 p-7 shadow-2xl backdrop-blur-sm"><p className="text-sm font-bold text-emerald-200">رسالتنا</p><p className="mt-4 text-2xl font-black leading-10">«{foundationData.tagline}»</p><div className="mt-8 border-t border-white/15 pt-6"><p className="text-sm leading-7 text-emerald-50/85">نتيح للمانحين والشركاء متابعة البرامج، والتعرف على مجالات التعاون، والاطلاع على ما توافق المؤسسة على نشره من مؤشرات ووثائق.</p><Link href="/transparency" className="mt-5 inline-block font-bold text-[#f4d78f] hover:underline">استكشف الشفافية والحوكمة ←</Link></div></aside></div>
      </section>
      {content.sectionOrder.map((sectionId) => sectionBlocks[sectionId])}
    </div>
  );
}
