...
import { HOMEPAGE_SECTION_IDS, HOMEPAGE_SECTION_LABELS, PROTECTED_NAVIGATION_IDS, mergeHomepageContent, mergeNavigationContent } from "@shared/content-studio-defaults";

type StudioProps = {
  settings: FirestoreSettings;
  saving?: boolean;
  onSave: (updates: Pick<FirestoreSettings, "homepage" | "navigation">) => Promise<void>;
};
...
export default function ContentStudioManager({ settings, saving, onSave }: StudioProps) {
  ...
  }, [settings.homepage, settings.navigation]);

  const editableOrder = useMemo(() => homepage.sectionOrder.filter((id) => id !== "priorities"), [homepage.sectionOrder]);
  ...
  const save = async () => {
    ...
    setNotice("");
    await onSave({ homepage, navigation });
  };
  ...
  return (
    <div className="space-y-6 rounded-3xl bg-white p-5 shadow-sm md:p-7">
      ...
      <section className="rounded-2xl border border-slate-200 p-5">
        ...
        <p className="mt-1 text-sm text-slate-600">يمكن إخفاء أي قسم من الواجهة أو تحريك ترتيبه. لا تنشأ أولوية تبرع من هذه البطاقة؛ استخدمي تبويب الأولويات.</p>
