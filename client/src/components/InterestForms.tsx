...
export function PartnerInquiryForm() {
  ...
  return (
    <section id="partner-inquiry" className="rounded-3xl border border-emerald-100 bg-emerald-50 p-6 md:p-8" aria-labelledby="partner-inquiry-title">
      ...
      <form onSubmit={submit} className="mt-6 grid gap-5 md:grid-cols-2" aria-describedby="partner-inquiry-help">
        ...
        <label className="flex items-start gap-3 text-sm leading-6 text-slate-700 md:col-span-2"><input type="checkbox" checked={formData.consent} onChange={(event) => setFormData((previous) => ({ ...previous, consent: event.target.checked }))} required className="mt-1 h-4 w-4 accent-emerald-700" />أوافق على استخدام بيانات الاتصال هذه لمتابعة طلب الشراكة فقط، وفق سياسة الخصوصية بالمؤسسة.</label>
        <Button type="submit" disabled={status === "sending"} className="md:col-span-2 bg-emerald-700 py-3 font-bold text-white hover:bg-emerald-800 disabled:cursor-not-allowed disabled:opacity-70">{status === "sending" ? "جاري الإرسال..." : "إرسال طلب الشراكة"}</Button>
      </form>
    </section>
  );
}
...
export function VolunteerApplicationForm() {
  ...
  return (
    <section className="rounded-3xl border border-emerald-100 bg-white p-6 shadow-sm md:p-8" aria-labelledby="volunteer-form-title">
      ...
      <form onSubmit={submit} className="mt-6 grid gap-5 md:grid-cols-2" aria-describedby="volunteer-form-help">
        ...
        <Button type="submit" disabled={status === "sending"} className="md:col-span-2 bg-emerald-700 py-3 font-bold text-white hover:bg-emerald-800 disabled:cursor-not-allowed disabled:opacity-70">{status === "sending" ? "جاري الإرسال..." : "إرسال طلب التطوع"}</Button>
      </form>
    </section>
  );
}
