...
export function PartnerInquiryForm() {
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const message = ["طلب شراكة من موقع مؤسسة مشروعنا إلى الجنة", `الجهة: ${formData.organizationName}`, `القطاع: ${formData.sector}`, `المسؤول: ${formData.contactName}`, `البريد: ${formData.email}`, formData.phone ? `الهاتف: ${formData.phone}` : "", `نوع التعاون: ${formData.cooperationType}`, formData.programInterest ? `مجال الاهتمام: ${formData.programInterest}` : "", formData.estimatedValue ? `النطاق التقديري: ${formData.estimatedValue}` : "", formData.timeline ? `الإطار الزمني: ${formData.timeline}` : "", formData.notes ? `ملاحظات: ${formData.notes}` : ""].filter(Boolean).join("\n");
  ...
  return (
    <section id="partner-inquiry" className="rounded-3xl border border-emerald-100 bg-emerald-50 p-6 md:p-8" aria-labelledby="partner-inquiry-title">
      ...
      {status === "success" && <p className="mt-5 rounded-xl border border-emerald-300 bg-white px-4 py-3 font-semibold text-emerald-800" role="status" aria-live="polite">فُتح WhatsApp لإرسال طلب الشراكة مباشرة. لا يُعد الطلب مرسلاً إلا بعد إرساله من WhatsApp.</p>}
      <form onSubmit={submit} className="mt-6 grid gap-5 md:grid-cols-2" aria-describedby="partner-inquiry-help" aria-busy={status === "sending"}>
        ...
        <Button type="submit" className="md:col-span-2 bg-emerald-700 py-3 font-bold text-white hover:bg-emerald-800">فتح WhatsApp لإرسال طلب الشراكة</Button>
      </form>
    </section>
  );
}
...
export function VolunteerApplicationForm() {
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    const message = ["طلب تطوع من موقع مؤسسة مشروعنا إلى الجنة", `الاسم: ${formData.name}`, `البريد: ${formData.email}`, formData.phone ? `الهاتف: ${formData.phone}` : "", `الوقت المتاح: ${formData.availability}`, `مجال المساهمة: ${formData.areaOfInterest}`, formData.skills ? `المهارات: ${formData.skills}` : "", formData.message ? `رسالة: ${formData.message}` : ""].filter(Boolean).join("\n");
    window.open(`${foundationData.contact.whatsapp}?text=${encodeURIComponent(message)}`, "_blank", "noopener,noreferrer");
    setStatus("success");
  };
  ...
  return (
    <section className="rounded-3xl border border-emerald-100 bg-white p-6 shadow-sm md:p-8" aria-labelledby="volunteer-form-title">
      ...
      {status === "success" && <p className="mt-5 rounded-xl border border-emerald-300 bg-emerald-50 px-4 py-3 font-semibold text-emerald-800" role="status" aria-live="polite">فُتح WhatsApp لإرسال طلب التطوع مباشرة. لا يُعد الطلب مرسلاً إلا بعد إرساله من WhatsApp.</p>}
      <form onSubmit={submit} className="mt-6 grid gap-5 md:grid-cols-2" aria-describedby="volunteer-form-help" aria-busy={status === "sending"}>
        ...
        <Button type="submit" className="md:col-span-2 bg-emerald-700 py-3 font-bold text-white hover:bg-emerald-800">فتح WhatsApp لإرسال طلب التطوع</Button>
      </form>
    </section>
  );
}
